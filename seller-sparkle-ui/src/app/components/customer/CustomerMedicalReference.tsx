...
export function CustomerMedicalReference({ title, value, onChange }: CustomerMedicalReferenceProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {value?.doctorId ? (
        <div className="overflow-hidden rounded-xl border border-teal-200 bg-teal-50/80 dark:border-teal-500/30 dark:bg-teal-500/10">
          {/* Doctor */}
          <div className="flex items-start justify-between gap-3 p-4">
            <div className="flex min-w-0 items-start gap-3">
              <div className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-teal-600 text-white shadow-sm dark:bg-teal-500">
                <Stethoscope className="h-4 w-4" />
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-teal-700 dark:text-teal-300">
                  <CheckCircle2 className="h-3.5 w-3.5" />
                  Doctor linked
                </div>
                <p className="mt-1 truncate text-base font-semibold text-teal-950 dark:text-teal-50">
                  {value.doctorName}
                </p>
                ...
                <p className="mt-2 inline-flex rounded-md bg-background/80 px-2 py-0.5 font-mono text-sm font-bold tracking-wider text-teal-900 ring-1 ring-teal-200/80 dark:bg-background/40 dark:text-teal-100 dark:ring-teal-500/30">
                  {value.uniqueCode}
                </p>
              </div>
            </div>
          ...
          {/* Hospitals */}
          <div className="border-t border-teal-200/70 bg-background/55 px-4 py-3.5 dark:border-teal-500/20 dark:bg-background/30">
            <div className="mb-2.5 flex items-center justify-between gap-2">
              <p className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-teal-800/70 dark:text-teal-300/80">
                <Building2 className="h-3.5 w-3.5" />
                Hospitals
              </p>
              {hospitals.length > 0 && (
                <span className="rounded-full bg-teal-100 px-2 py-0.5 text-[10px] font-bold tabular-nums text-teal-800 dark:bg-teal-500/20 dark:text-teal-200">
                  {hospitals.length}
                </span>
              )}
            </div>
            ...
            {hospitals.length > 0 ? (
                    ...
                    >
                      ...
                      {detail && (
                        <p className="mt-1 flex items-start gap-1.5 text-xs leading-snug text-muted-foreground">
                          <MapPin className="mt-0.5 h-3.5 w-3.5 shrink-0 text-teal-600 dark:text-teal-400" />
                          <span>{detail}</span>
                        </p>
                      )}
                    </li>
              ...
              <p className="rounded-lg border border-dashed border-teal-200/80 bg-teal-50/40 px-3 py-2.5 text-xs text-teal-800/70 dark:border-teal-500/30 dark:bg-teal-500/5 dark:text-teal-200/70">
