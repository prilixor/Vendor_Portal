...
import { mockAuditLogs } from "@/app/services/mockData";
...
const AuditLogs = () => {
  ...
  const actors = Array.from(new Set(mockAuditLogs.map((l) => l.actor)));

  const filtered = mockAuditLogs.filter((l) => {
    const m = actor === "all" || l.actor === actor;
    const s = !search || l.action.toLowerCase().includes(search.toLowerCase()) || l.entityType.toLowerCase().includes(search.toLowerCase());
    return m && s;
  });
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((log) => (
                <tr key={log.id} className="hover:bg-muted/20">
                  <td className="px-4 py-3"><span className="rounded-md bg-primary-soft px-2 py-1 text-xs font-mono font-semibold text-primary">{log.action}</span></td>
                  ...
                  <td className="px-4 py-3 text-xs text-muted-foreground">{format(new Date(log.timestamp), "MMM d, yyyy HH:mm")}</td>
                </tr>
              ))}
            </tbody>
          </table>
