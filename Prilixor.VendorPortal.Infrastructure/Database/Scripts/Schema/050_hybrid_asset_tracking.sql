...
-- Guarded for multi-database execution (vendor_portal_db & customer_portal_db)

DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'vendor_product_listings') THEN
...
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'customer_rental_orders') THEN
