...
class _LegalReconsentGateState extends State<LegalReconsentGate> {
  ...
  void didChangeDependencies() {
    ...
    final key = auth.isAuthenticated ? (vendorId ?? '') : 'out';
    if (key == _sessionKey) return;
    _sessionKey = key;
    _pending = [];
    _agreed = false;
    _error = null;
    if (auth.isAuthenticated && vendorId != null && vendorId.isNotEmpty) {
      _loaded = false;
  ...
  Future<void> _load() async {
    ...
    final vendorId = auth.vendorId;
    if (!auth.isAuthenticated || vendorId == null || vendorId.isEmpty) return;
    try {
      final res = await ApiClient().dio.get('/vendors/$vendorId/legal-reconsent');
  ...
  Future<void> _accept() async {
    ...
    try {
      await ApiClient().dio.post('/vendors/$vendorId/legal-acceptances', data: {
        'vendorId': vendorId,
        'screen': 'reconsent',
        'acceptedLegal': true,
